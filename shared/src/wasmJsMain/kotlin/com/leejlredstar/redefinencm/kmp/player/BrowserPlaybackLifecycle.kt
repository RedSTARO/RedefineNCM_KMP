 ...
 *
 * A hidden tab is not a page going away, so `visibilitychange` to hidden does not stop playback:
 * stopping there would pause the music whenever the user switches tabs or minimises the browser.
 * Desktop browsers keep a hidden tab's audio running; mobile Safari may still suspend the page
 * on lock regardless.
 */
