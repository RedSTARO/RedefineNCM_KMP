...
# Emits iosApp/Configuration/Version.xcconfig from the Git-derived app version.
#
# This must be an xcconfig rather than a build phase that rewrites the built Info.plist:
# the app target has an AppIcon asset catalog, so actool produces a partial Info.plist and
# Xcode schedules ProcessInfoPlistFile *after* every build phase (and after the
# Embed App Extensions copy). A script phase can therefore never observe the built plist —
# declaring it as an input just creates a dependency cycle. Build settings, by contrast, are
# resolved before ProcessInfoPlistFile runs and are substituted into both Info.plists.
#
# Run this before xcodebuild. SRCROOT is set by Xcode; standalone runs derive it.
...
XCCONFIG_PATH="$SRCROOT/Configuration/Version.xcconfig"
cat > "$XCCONFIG_PATH" <<EOF
// Generated by iosApp/Scripts/stamp-version.sh — do not edit, do not commit.
MARKETING_VERSION = $BASE_VERSION
CURRENT_PROJECT_VERSION = $VERSION_CODE
APP_VERSION_NAME = $VERSION_NAME
EOF

echo "Wrote $XCCONFIG_PATH for version $VERSION_NAME (build $VERSION_CODE)."
