...
> 在 macOS 26 / Xcode 26.5 上补齐。Kotlin 源码编译通过不代表 framework 能链接：
> `object : NSObject()` 这类错误只在 codegen 阶段暴露，必须有 macOS 才能发现。
...
- [x] **Web / wasmJs 行为对齐**（2026-07-11）：声明 browser executable target；复用全部
      ...
      同时取消待完成的播放地址解析。
- [x] `Platform.isDesktop/isMobile` 接口与四个平台的 actual 对齐（2026-07-11）
...
- [x] **iOS 构建与验证**（2026-08-21，macOS 26 / Xcode 26.5）：framework 链接、`iosSimulatorArm64Test`
      ...
      不再是 fallback。尚未做：真机上的运行时行为（AVPlayer 播放、Live Activity/灵动岛、
...
> 本节保留作历史记录。JavaFX WebKit 于 2026-07-05 淘汰，Windows x64 WebView2 于 2026-07-27
...
页内 `#desktop-console` 播放控制台也一并删除，桌面端改用与 Android 相同的控制岛
`AutoHideMiniPlayerController`。控制岛是页内控制台的严格超集，另有折叠态、歌词详情展开
与封面取色。
...
| 控制岛 | ✅ `AutoHideMiniPlayerController` 四端同一份，统一按 Android 基准 3600 ms 自动收起；桌面端展开态额外有独占的输出音量滑块 |
...
- Web 首次播放受用户手势/自动播放策略约束；自动播放被拒绝时保持暂停。Media Session 与系统通知
