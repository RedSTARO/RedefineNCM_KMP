...
> 最后更新：2026-07-04　｜　状态：**功能迁移基本完成（Android + common 全对齐原版）**；Android + Desktop 构建验证通过；iOS 代码就绪但需 macOS 构建。
...
| 启动更新检查（checkUpdate 设置 + GitHub releases） | ✅（MainViewModel.checkAppUpdate → Snackbar；Android 读 versionName，其他端跳过） |
...
| 歌单批量下载（DownloadManager，5 首/批，跳过已存在） | ✅（common 编排 + Android actual；其他端 stub） |
...
| 封面取色（Palette muted→vibrant→dominant） | ✅ Android=Palette；JVM=Skia 采样近似；iOS=stub |
| 实况通知歌词（歌词作标题 + Android 16 Live Update + 媒体按钮） | ✅（KMP 版含 prev/toggle/next action，超出原版） |
...
### B. 需要外部环境
- [ ] **iOS 构建与验证**：需 macOS + Xcode 16+（AVPlayer、Live Activity Swift 源码已就绪；SongDownloader/取色/更新检查为 stub）
