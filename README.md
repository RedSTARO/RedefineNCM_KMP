...
- `/scrobble/v1` 在有效播放进度达到歌曲时长一半后至多尝试一次，拖动、缓冲、卡住和
  系统休眠时间不计入；
- 后端明确以 HTML 404 表示没有 `/scrobble/v1` 时，客户端会缓存该能力并改用两段
  `POST /weblog`（`startplay` → `play`）。兼容路径要求正整数歌单来源 ID，并且只给这两次
  请求的 Cookie 快照派生 `os=osx`，不会改写设置中的 Cookie；
...
2026-07-13 使用真实账号对局域网 `4.30.2` 服务做了受控验证：旧 `/scrobble` 虽返回
`code=200`，账号周记录、最近播放和等级快照均未变化；两段 `/weblog` 兼容上报在约 3 秒后由
周记录和最近播放共同确认生效。未派生 `os=osx` 的对照请求没有产生账号侧变化。

当前服务仍没有 `/relay/play/state/submit`。即使当前上游版本也只有 relay 的 submit，没有拉取
当前 `progress/sessionId/playMode` 的接口；所以这里只能提交远端状态，无法实现官方客户端级别的
